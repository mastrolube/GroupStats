...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "ui_groupstats.ui")
)
...
class GroupStatsDialog(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.layer.currentIndexChanged.connect(
            self.layerSelection
        ...
        self.ui.actionCopySelected.triggered.connect(
            self.copyMarked
        )  # Layer selection signal
        self.ui.actionSaveCSV.triggered.connect(
            self.exportToCSV
        )  # Layer selection signal
        self.ui.actionSaveCSVSelected.triggered.connect(
            self.exportMarkedToCSV
        )  # Layer selection signal
        self.ui.actionShowPanel.triggered.connect(
            self.showControlPanel
        )  # Layer selection signal
        self.ui.actionShowOnMap.triggered.connect(
            self.showOnMap
        )  # Layer selection signal
        ...
        self.ui.result.verticalHeader().sortIndicatorChanged.connect(
            self.sortRows
        )  # Layer selection signal
    ...
    def blockCalculations(self, x, y, z):  # finished
        ...
        if (
            "geometry" in [a[0] for a in values]
            or "countAttributes" in [a[0] for a in values]
        ) and "calculations" in [a[0] for a in values + rows + columns]:
            self.ui.calculate.setEnabled(True)
        elif (
            "attributeTxt" in [a[0] for a in values]
            and len([a for a in values + rows + columns if a[0] == "calculations"]) > 0
        ):
            if {
                a[2] for a in values + rows + columns if a[0] == "calculations"
            }.issubset(
                set(self.calculations.textList)
            ):  # [a for a in values+rows+columns if a[0]=='calculations'][0][2]==0:
                self.ui.calculate.setEnabled(True)
        else:
            self.ui.calculate.setEnabled(False)

    def calculate_geod_length(self, geometry):
        length = 0
        if geometry.isMultipart():
            for part in geometry.asMultiPolyline():
                length += self.distance_area.measureLength(part)
        else:
            length = self.distance_area.measureLength(geometry)
        return length

    def calculate_geod_area(self, geometry):
        return self.distance_area.measureArea(geometry)
    ...
    def showScore(self):  # finished
        ...
        value = [x for x in chosenValues if x[0] != "calculations"][0]
        if value[0] == "geometry":
            if value[2] == 1:
                valueFunction = lambda _object: self.calculate_geod_length(
                    _object.geometry()
                )
            elif value[2] == 2:
                valueFunction = lambda _object: self.calculate_geod_area(
                    _object.geometry()
                )
        elif value[0] == "attributeTxt":
            valueFunction = lambda _object: (
                None
                if _object.attribute(value[1]) is None
                else _object.attribute(value[1])
            )
            ...
            valueFunction = lambda _object: (
                ...
                if _object.attribute(value[1]) is None
                or (
                    isinstance(_object.attribute(value[1]), QVariant)
                    and _object.attribute(value[1]).isNull()
                )
                else (
                    float(_object.attribute(value[1]).value())
                    if isinstance(_object.attribute(value[1]), QVariant)
                    else float(_object.attribute(value[1]))
                )
            )
        ...
        if _filter := self.ui._filter.toPlainText():
            request.setFilterExpression(_filter)
        iterator = layer.getFeatures(request)

        if (
            self.ui.onlySelected.isChecked()
        ):  # Retrieve the IDs of the selected _objects
        ...
        while iterator.nextFeature(f):  # for each object ...
            if not onlySelected or (onlySelected and (f.id() in selectedObjects)):
                ...
                if valueToCalculate != None or self.ui.useNULL.isChecked():
                    ...
                    if key in result:
                        result[key][0].append(
                            valueToCalculate
                        )  # if key exists, a new value is added to the list
                    else:
                        result[key] = [
                            [valueToCalculate],
                            [],
                        ]  # if key does not exist then a new list is created
                ...
                self.statusBar().showMessage(
                    ...
                    + "%.0f%%" % ((float(counter) / float(numberOfObjects)) * 100)
                )  # Displaying progress
        ...
        calculations = [
            [
                x[2] for x in chosenValues if x[0] == "calculations"
            ],  # list of selected calculations in values, rows and columns
            [x[2] for x in chosenRows if x[0] == "calculations"],
            [x[2] for x in chosenColumns if x[0] == "calculations"],
        ]

        if (
            len(calculations[0]) != 0
        ):  # Take to calculations only the non-empty part of the list above
        ...
        data = [
            max(len(columns), len(columns) * len(calculations[2])) * [("", ())]
            for _ in range(max(len(rows), len(rows) * len(calculations[1])))
        ]
        for x in keys:  # Calculation of values ​​for all keys
            nrw = rowDictionary[x[0]]  # rows no in the data table for the chosen key
            nrk = columnDictionary[
                x[1]
            ]  # column number in the data table for the chosen key
            for n, y in enumerate(calculation):  # making all calculates for all keys
                if len(calculations[1]) > 0:
                    data[nrw * len(calculations[1]) + n][nrk] = [
                        self.calculations.list[y][1](result[x][0]),
                        result[x][1],
                    ]  # insert result if calculations with row
                elif len(calculations[2]) > 0:
                    data[nrw][nrk * len(calculations[2]) + n] = [
                        self.calculations.list[y][1](result[x][0]),
                        result[x][1],
                    ]  # insert result if calculations from columns
                else:
                    data[nrw][nrk] = [
                        self.calculations.list[y][1](result[x][0]),
                        result[x][1],
                    ]  # insert result if calculations with values
        ...
        if len(rows1) > 0 and len(columns1) > 0:
            ...
            for i in range(len(columns1[0]), 0, -1):
                self.ui.result.verticalHeader().setSortIndicator(
                    i - 1, Qt.AscendingOrder
                )
            for i in range(len(rows1[0]), 0, -1):
                self.ui.result.horizontalHeader().setSortIndicator(
                    i - 1, Qt.AscendingOrder
                )
            ...
            rekordy = "record" if NULLcounter == 1 else "records"
            if self.ui.useNULL.isChecked() and NULLcounter > 0:
                textNULL = QCoreApplication.translate(
                    "GroupStats",
                    f'  (used {NULLcounter} {rekordy} with null value in "{value[1]}" field)',
                )
            elif self.ui.useNULL.isChecked() == False and NULLcounter > 0:
                textNULL = QCoreApplication.translate(
                    "GroupStats",
                    f'  (not used {NULLcounter} {rekordy} with null value in "{value[1]}" field)',
                )
            else:
                textNULL = ""
            ...
            self.statusBar().showMessage(
                QCoreApplication.translate("GroupStats", "No data found."), 10000
            )
    ...
    def setLayers(self, layer):  # finished
        ...
        index = self.ui.layer.currentIndex()
        if index != -1:
            layerId = self.ui.layer.itemData(
                index
            )  # id of the previously selected layer
        ...
        if index != -1:
            index2 = self.ui.layer.findData(
                layerId
            )  # if the previously selected layer is a list then select it
            if index2 != -1:
    ...
    def setFilter(self):  # finished 2
        ...
        text = (
            self.ui._filter.toPlainText()
        )  # Retrieve the text from the window and display the query window
    ...
    def downloadDataFromTheTable(self, allData=True, controlCharacters=False):
        ...
        if (
            allData == False
        ):  # If the option 'only checked' get indexes of selected fields
    ...
    def showOnMap(self):  # change not to duplicate indexes from cells
        indexList = (
            self.ui.result.selectedIndexes()
        )  # Retrieve the indexes of the selected fields
        idList = []
        ...
        try:
            ...
            QMessageBox.information(
                None,
                QCoreApplication.translate("GroupStats", "Information"),
                QCoreApplication.translate("GroupStats", "No data selected"),
            )
        else:
            ...
            if (
                len(idList) == 1 and self.tm5.layer.geometryType() == 0
            ):  #      if the layer is point and there is only one object in the group ..
...
class ModelList(QAbstractListModel):
    ...
    def mimeTypes(self):
        return [
            "application/x-groupstats-polaL",
            "application/x-groupstats-polaWK",
            "application/x-groupstats-polaW",
        ]
    ...
    def mimeData(self, indexy, typMime="application/x-groupstats-polaL"):
        ...
        for index in indexy:
            ...
            stream.writeBytes(
                bytes(self._data[row][0], "utf-8")
                if isinstance(self._data[row][0], str)
                else bytes(self._data[row][0])
            )
            stream.writeBytes(
                bytes(self._data[row][1], "utf-8")
                if isinstance(self._data[row][1], str)
                else bytes(self._data[row][1])
            )
...
class ModelRowsColumns(ModelList):
    ...
    def mimeData(self, indexy):
        return super(ModelRowsColumns, self).mimeData(
            indexy, "application/x-groupstats-polaWK"
        )
    ...
    def dropMimeData(self, dataMime, share, row, column, index):
        ...
        while not stream.atEnd():
            ...
            if (
                typ == "calculations"
                and typ in [x[0] for x in dataWKiW]
                and dataType == "application/x-groupstats-polaL"
            ):
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                        "GroupStats", "Function can be dropped in only one area"
                    ),
                    15000,
                )
                return False
            elif (field in self.modelWiK or field in self._data) and dataType in [
                "application/x-groupstats-polaL",
                "application/x-groupstats-polaW",
            ]:
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                        "GroupStats", "This field has already been droped"
                    ),
                    15000,
                )
                return False
            ...
            #    return False
                ...
                typ == "calculations"
                and id not in self.calculations.textList
                and "attributeTxt" in [x[0] for x in self.valueModel]
            ):  # name != self.calculations.lista[0][0]
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                        "GroupStats",
                        "For the text value function can only be one of (%s)"
                        % self.calculations.textNames,
                    ),
                    15000,
                )
                return False
...
class ValueModel(ModelList):
    ...
    def mimeData(self, indexy):
        return super(ValueModel, self).mimeData(
            indexy, "application/x-groupstats-polaW"
        )
        ...
        while not stream.atEnd():
            ...
            if len(self._data) >= 2:
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                        "GroupStats",
                        "Area 'Value' may contain a maximum of two entries",
                    ),
                    15000,
                )
                return False
                ...
                typ == "calculations"
                and typ in [x[0] for x in allData]
                and dataType == "application/x-groupstats-polaL"
            ):
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                        "GroupStats", "Function can be droped in only one area"
                    ),
                    15000,
                )
                return False
                ...
                len(self._data) == 1
                and typ != "calculations"
                and self._data[0][0] != "calculations"
            ):
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                        "GroupStats",
                        "In the area 'Value' one of the items must be a function",
                    ),
                    15000,
                )
                return False
            elif len(self._data) == 1 and (
                (
                    typ == "attributeTxt"
                    and self._data[0][2] not in self.calculations.textList
                )
                or (
                    id not in self.calculations.textList
                    and self._data[0][0] == "attributeTxt"
                )
            ):
                self.mainWindow.statusBar().showMessage(
                    QCoreApplication.translate(
                ...
                and len(
                    [
                        ...
                        if (
                            x[0] == "calculations"
                            and x[2] not in self.calculations.textList
                        )
                    ]
                )
                > 0
...
class ResultModel(QAbstractTableModel):  # finished
    ...
    def __init__(self, data, rows, columns, layer, parent=None):
        ...
        self.offsetX = max(
            1, len(rows[0])
        )  # Coordinate shift so that the data starts with 0.0
        self.offsetY = max(1, len(columns[0]))

        if (
            len(rows[0]) != 0 and len(columns[0]) != 0
        ):  # One line offset (empty) to make room for the names of the lines
            self.offsetY += 1
        ...
        if role == Qt.DisplayRole:
            if row >= 0 and column >= 0:  # Data
                ...
                if len(self.rows[0]) > 0:
                    ...
                    return self.columns[column + 1][
                        row
                    ]  # descriptions and column names if there is no gap line
    ...
    def sort(self, column, mode):
                ...
                tmp.extend(
                    [
                        (
                            ...
                            (
                                float("nan")
                                if not str(d[column - self.offsetX][0]).strip()
                                else float(d[column - self.offsetX][0])
                            ),
                        )
        ...
        for (
            ...
            tmp
        ):  # Arrange all data and row descriptions according to a temporary sort list
    ...
    def sortRows(self, row, mode):
        ...
        """
        if (
            len(self.columns) - self.offsetX <= 1
        ):  # If there is only one column, there is nothing to sort
        ...
        if row >= self.offsetY:  # Selecting data to sort
            ...
            try:
                tmp.extend(
                    [
                        (n, (float("nan") if not str(d[0]).strip() else float(d[0])))
                        for n, d in enumerate(self._data[row - self.offsetY])
                    ]
                )
            except (ValueError, TypeError):
                tmp.extend(
                ...
                )  # n-column number before storting, d-data in the column
        ...
        self.columns.append(
            columns2[0]
        )  # Adding column names (only names, no column descriptions)
        ...
        for (
            ...
            tmp
        ):  # Arrangement of column descriptions according to a temporary sort list
...
def sort_values_nulls_last(alist, rev):
    ...
    contains_values = list(
        sorted(contains_values, key=lambda x: x[1], reverse=True if rev == 1 else False)
    )
...
class WindowResults(QTableView, QMenu):
    ...
    def contextMenuEvent(self, event):
        ...
        selectionIndex = self.indexAt(
            event.pos()
        )  # Get the index at the position of the right click event
        if selectionIndex.row() in [0, -1] or selectionIndex.column() in [
            0,
            -1,
        ]:  # Don't do anyting if row or column header is clicked
            return
        selectedCell = str(
            self.indexAt(event.pos()).data()
        )  # Get the content of the cell on right click
        copyCellContentAction.triggered.connect(
            lambda: GroupStatsDialog().copyCellContent(selectedCell)
        )
        self.menu.addAction(copyCellContentAction)
        self.selectionModel().select(
            selectionIndex, QItemSelectionModel.ClearAndSelect
        )  # Clear any selection in the table and select the single cell that has been clicked
    ...
    def selectionCommand(self, index, event=None):
        ...
        """
        flag = super(WindowResults, self).selectionCommand(
            index, event
        ...
        if self.model():
            test = self.model().data(
                index, Qt.UserRole + 1
            )  # checking the selected cell type
        if test == "row":
    ...
    def _selectAll(self, index):
        ...
        if test not in ("data", "row", "column"):  # checking if the corner
            if self.selectionModel().isSelected(
                index
            ):  # if the corner is selected, it also marks all dataa
                self.selectAll()
            else:
...
class Calculations(QObject):  # finished
    ...
    def __init__(self, parent):  # List with ID, name and calculating function
        ...
        self.list = {
            0: (QCoreApplication.translate("Calculations", "count"), self.count),
            ...
            4: (
                QCoreApplication.translate("Calculations", "stand.dev."),
                self.stand_dev,
            ),
